...
sys.path.append('../')
from webpage.main import app, db
...
class UsersTests(unittest.TestCase):
    ...
    def test_invalid_username_registration(self):
        ...
        response2 = self.register('superlongusernameisinvalid', 'user@email.com', 'password')
        self.assertIn(b'Field must be between 2 and 20 characters long.', response.data)

    def test_invalid_email_registration(self):
        response = self.register('test2', 'test@example', 'FlaskIsAwesome')
        self.assertIn(b'Invalid email address.', response.data)
        response = self.register('test3', 'testemailatemail.com', 'FlaskIsAwesome')
        self.assertIn(b'Invalid email address.', response.data)
